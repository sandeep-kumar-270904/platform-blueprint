...
import { Search as SearchIcon, Calendar, ArrowRight, BookOpen, UserPlus, UsersRound } from "lucide-react";
...
const Search = () => {
  ...
  const [results, setResults] = useState<{ colleges: any[]; events: any[]; courses: any[]; providers?: any[]; roommateProfiles?: any[]; roommateGroups?: any[] }>({ colleges: [], events: [], courses: [] });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 pt-24 pb-12">
        <div className="mb-8 border-b pb-6">
          ...
          {query ? (
            <div className="flex gap-2 flex-wrap">
              <div className="text-muted-foreground">Showing results for</div>
              <div className="font-semibold px-2 bg-secondary rounded text-foreground">"{query}"</div>
            </div>
          ) : (
            <p className="text-muted-foreground">Enter a search term above to find colleges, events, courses and more.</p>
          )}
        </div>

        {(query.toLowerCase().includes('roommate') || query.toLowerCase().includes('flatmate') || query.toLowerCase().includes('connections')) && (
          <div className="mb-12">
            <h2 className="text-2xl font-bold mb-6">Quick Links</h2>
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              <Card className="hover:shadow-md transition-shadow cursor-pointer flex flex-col h-full overflow-hidden border-primary/20 bg-primary/5" onClick={() => navigate('/roommates')}>
                <CardContent className="p-6 flex flex-col flex-1 items-center text-center">
                  <div className="h-16 w-16 rounded-full bg-primary/10 flex items-center justify-center mb-4">
                    <UserPlus className="h-8 w-8 text-primary" />
                  </div>
                  <h3 className="font-semibold text-lg mb-2">Find Roommates</h3>
                  <p className="text-sm text-muted-foreground mb-4">
                    Search and match with compatible roommates based on your preferences.
                  </p>
                  <div className="mt-auto w-full pt-4 flex items-center justify-center text-primary font-medium text-sm">
                    Open Finder <ArrowRight className="h-4 w-4 ml-1" />
                  </div>
                </CardContent>
              </Card>

              <Card className="hover:shadow-md transition-shadow cursor-pointer flex flex-col h-full overflow-hidden border-indigo-500/20 bg-indigo-500/5" onClick={() => navigate('/roommates/connections')}>
                <CardContent className="p-6 flex flex-col flex-1 items-center text-center">
                  ...
                  <h3 className="font-semibold text-lg mb-2">My Connections</h3>
                  <p className="text-sm text-muted-foreground mb-4">
                    Manage your saved profiles, active requests, and current connections.
                  </p>
                  <div className="mt-auto w-full pt-4 flex items-center justify-center text-indigo-500 font-medium text-sm">
                    Open Connections <ArrowRight className="h-4 w-4 ml-1" />
                  </div>
                </CardContent>
              </Card>
            </div>
        ...
        {loading ? (
        ...
        ) : (results.colleges.length === 0 && results.events.length === 0 && results.courses.length === 0 && (!results.providers || results.providers.length === 0) && (!results.roommateProfiles || results.roommateProfiles.length === 0) && (!results.roommateGroups || results.roommateGroups.length === 0)) ? (
          ...
          <div className="space-y-12">
            ...
            {results.roommateProfiles && results.roommateProfiles.length > 0 && (
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold">Roommate Profiles</h2>
                  <span className="text-sm text-muted-foreground font-medium bg-muted px-2 py-1 rounded-md">{results.roommateProfiles.length} found</span>
                </div>
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                  {results.roommateProfiles.map(profile => (
                    <Card key={profile._id} className="hover:shadow-md transition-shadow cursor-pointer flex flex-col h-full overflow-hidden" onClick={() => navigate(`/roommates?profile=${profile._id}`)}>
                      <CardContent className="p-4 flex flex-col flex-1 items-center text-center mt-4">
                        ...
                        <h3 className="font-semibold line-clamp-1 mb-1">{profile.user?.name || profile.user?.full_name || 'Roommate'}</h3>
                        <p className="text-sm text-muted-foreground line-clamp-2 mb-4 flex-1">
                          {profile.bio || `Budget: $${profile.budgetRange?.min || 0} - $${profile.budgetRange?.max || 0}`}
                        </p>
                        <div className="mt-auto w-full pt-4 flex items-center justify-center text-primary font-medium text-sm border-t">
            ...
            {results.roommateGroups && results.roommateGroups.length > 0 && (
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold">Roommate Groups</h2>
                  <span className="text-sm text-muted-foreground font-medium bg-muted px-2 py-1 rounded-md">{results.roommateGroups.length} found</span>
                </div>
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                  {results.roommateGroups.map(group => (
                    <Card key={group._id} className="hover:shadow-md transition-shadow cursor-pointer flex flex-col h-full overflow-hidden" onClick={() => navigate(`/roommates?group=${group._id}`)}>
                      <CardContent className="p-4 flex flex-col flex-1 items-center text-center mt-4">
                        <div className="h-20 w-20 rounded-full bg-indigo-500/10 flex items-center justify-center mb-4">
                          <UsersRound className="h-10 w-10 text-indigo-500" />
                        </div>
                        <h3 className="font-semibold line-clamp-1 mb-1">{group.name}</h3>
                        <p className="text-sm text-muted-foreground mb-4 flex-1">
                          {group.status} • {group.targetSize} members
                        </p>
                        <div className="mt-auto w-full pt-4 flex items-center justify-center text-indigo-500 font-medium text-sm border-t">
                          View Group <ArrowRight className="h-4 w-4 ml-1" />
                        </div>
                      </CardContent>
                    </Card>
                  ))}
