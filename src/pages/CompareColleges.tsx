        ...
        <div className="overflow-x-auto rounded-xl border border-border bg-card shadow-sm">
          <table className="w-full text-left border-collapse min-w-[800px]">
              ...
              <tr>
                ...
                {colleges.map(c => (
                  <th key={c._id} className="p-4 border-b border-r border-border bg-card min-w-[250px] align-top text-center sticky top-0 z-10">
                    <div className="text-4xl mb-3">{c.logoOrIcon || "🏛️"}</div>
            ...
            <tbody>
              ...
              {/* Rating */}
              <tr>
                <th className="p-4 border-b border-r border-border bg-muted/50 font-semibold sticky left-0 z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] text-sm">Rating</th>
                {colleges.map(c => renderCell(
                  <div className="flex items-center justify-center gap-1">
                    <span className="text-lg">{c.rating?.toFixed(1) || "N/A"}</span>
                    <span className="text-muted-foreground text-xs">({c.totalReviews})</span>
                  </div>,
