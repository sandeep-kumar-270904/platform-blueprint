...
vitals_card = """
                    {selectedApp.vitals && (
                      <Card className="mt-4 border-blue-200 dark:border-blue-900/50 bg-blue-50/50 dark:bg-blue-900/10">
                        <CardHeader className="pb-2">
                          <CardTitle className="text-sm text-blue-800 dark:text-blue-300">Candidate Vitals</CardTitle>
                        </CardHeader>
                        <CardContent className="flex gap-4 text-sm flex-wrap">
                          <div className="flex flex-col">
                            <span className="text-muted-foreground text-xs">Experience</span>
                            <span className="font-semibold">{selectedApp.vitals.yearsOfExperience} yrs</span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-muted-foreground text-xs">Verified Skills</span>
                            <span className="font-semibold">{selectedApp.vitals.verifiedSkillsCount}</span>
                          </div>
                          <div className="flex flex-col">
                            <span className="text-muted-foreground text-xs">Certifications</span>
                            <span className="font-semibold">{selectedApp.vitals.certificationsCount}</span>
                          </div>
                          {selectedApp.vitals.atsScore !== undefined && (
                            <div className="flex flex-col">
                              <span className="text-muted-foreground text-xs">ATS Match</span>
                              <span className="font-semibold">{selectedApp.vitals.atsScore}/100</span>
                            </div>
                          )}
                        </CardContent>
                      </Card>
...
if "selectedApp.vitals" not in content:
    # Insert right after: <h2 className="text-xl font-bold mb-4">Resume View</h2>
    # or after <Tabs defaultValue="resume">...
    # Let's insert before the PDFRenderer. Let's see how ATSDashboard is structured.
    # Usually we have `<div className="mt-6 border rounded-lg overflow-hidden h-[800px]">`
    # Let's just insert it right before `<div className="mt-6 border`

    content = re.sub(
        r'(<div className="mt-6 border[^>]*>)',
        vitals_card + r'\n                \1',
        content
    )
    with open(ats_path, "w", encoding="utf-8") as f:
        f.write(content)
    print("Updated ATSDashboard.tsx with Vitals Card")


# 2. OfferComparisonModal.tsx
offer_modal = """import React, { useState } from 'react';
...
with open("src/components/resume/OfferComparisonModal.tsx", "w", encoding="utf-8") as f:
    f.write(offer_modal)
print("Created OfferComparisonModal.tsx")
