...
async function runConcurrencyTests() {
  ...
  // Create mock data
  await User.deleteMany({ email: { $in: ['concurrent_student@test.com', 'concurrent_alum@test.com'] } });
  const student = await new User({ email: 'concurrent_student@test.com', password: 'password', role: 'student', full_name: 'Concurrent Student' }).save();
  const alumni = await new User({ email: 'concurrent_alum@test.com', password: 'password', role: 'user', full_name: 'Concurrent Alumni' }).save();
  ...
  try {
    ...
    // 1. Connection Request Race Condition Test (Priority 2)
    const requestPromises = Array(10).fill().map((_, i) => 
    ...
    const blocks = responses.filter(r => r.status === 400).length;
    if (successes !== 1) {
      console.log("Responses statuses:", responses.map(r => r.status));
      for (const res of responses) {
        if (res.status === 500) console.log(await res.text());
      }
    }

    check("Exactly 1 request succeeded", successes === 1);
    check("Other 9 requests were blocked", blocks === 9);
